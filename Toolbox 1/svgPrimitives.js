...
const createCircle = (cx, cy, r, fill, stroke, opacity = 1, rotate = 0) => {
    return `<circle cx="${cx}" cy="${cy}" r="${r}" fill="${fill}" stroke="${stroke}"  fill-opacity="${opacity}" transform="rotate(${rotate} ${cx} ${cy})"/>`;
}
